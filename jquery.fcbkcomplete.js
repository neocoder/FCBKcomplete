 ...
 * getData          - callback function to getData. if defined json_url is not used
...
(function( $, undefined ) {
	$.fn.fcbkcomplete = function(opt) {
		...
		function makeFcbkcomplete(el) {
			...
			function createFCBK() {
				var orignalClasses = options.preserveClasses ? element.get(0).className : '';
				holder = $('<ul class="holder fcbkcomplete '+orignalClasses+'"></ul>');

				options.width && holder.width(options.width);
				...
				complete = $('<div class="facebook-auto">').width(holderWidth);
				...
				feed = $('<ul id="'+elemid+'_feed"></ul>').width(holderWidth);
			...
			function addItem(title, value, preadded, locked, focusme) {
				...
				var aclose = $('<a class="closebutton" href="#">&times;</a>');
			...
			function load_feed(etext){
				counter = 0;
				if ( (options.json_url || options.getData) && maxItems()) {
					if (options.cache && json_cache_object.get(etext)) {
						...
						setTimeout( function() {
							...
							if ( typeof options.getData === 'function' ) {
								options.getData(etext, onData);
							} else {
								$.getJSON(options.json_url, {"tag": xssDisplay(etext)}, onData);
							}							
